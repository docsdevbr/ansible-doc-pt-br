...
.. glossary::
    ...
    Ansible Galaxy
        An `online distribution server <https://galaxy.ansible.com/ui/>`_ for finding and sharing Ansible community content, sometimes referred to as community Galaxy. Also, the command-line utility that lets users install individual Ansible Collections, for example ``ansible-galaxy collection install community.crypto``.
