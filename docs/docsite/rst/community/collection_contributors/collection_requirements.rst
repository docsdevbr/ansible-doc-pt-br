...
* The content that MUST be licensed with a free software license that is **compatible with** the `GPL-3.0-only <https://www.gnu.org/licenses/gpl-3.0-standalone.html>`_:
  ...
  * Code outside ``plugins/``: if it DOES NOT import code licensed under ``GPL-3.0-only`` it may be licensed under another license compatible with ``GPL-3.0-only``.
  * Non-code content.
  * To be allowed, the license MUST be considered open source and compatible with ``GPL-3.0-only`` on **both**:
...
* The content that MUST be licensed with the `GPL-3.0-only <https://www.gnu.org/licenses/gpl-3.0-standalone.html>`_:

  * All other code in the ``plugins/`` directory except code under the ``modules/`` and ``module_utils/`` directories (see above): these plugins are run inside of the Ansible controller process which is licensed under the ``GPL-3.0-only`` and often must import code from the controller. For these reasons, ``GPL-3.0-only`` MUST be used.
  * Code outside ``plugins/``: if it imports any other code that is licensed under ``GPL-3.0-only``. Note that this applies in particular to unit tests that often import code from ansible-core, ``plugins/``, ``module_utils/``, or ``modules/``, and such code is often licensed under ``GPL-3.0-only``.
